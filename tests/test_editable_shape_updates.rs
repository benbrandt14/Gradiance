...
fn test_editable_shape_update_triggers_mesh_update() {
    ...
    // 1. Spawn Box (10x10)
    let shape = ShapeType::Box { width: 10.0, height: 10.0 };
    ...
    assert!(entity_ref.contains::<Collider>(), "Collider should be generated");
    assert!(entity_ref.contains::<Mesh3d>(), "Mesh3d should be generated");

    let initial_mesh_handle = entity_ref.get::<Mesh3d>().unwrap().0.clone();

    // 2. Modify Shape (to Box 2x2)
    let new_shape = ShapeType::Box { width: 2.0, height: 2.0 };
    {
    ...
    assert_ne!(initial_mesh_handle, final_mesh_handle, "Mesh handle should change when shape changes");
    ...
    assert!((width - 2.0).abs() < 0.1, "Collider width should be approx 2.0, got {}", width);
}
