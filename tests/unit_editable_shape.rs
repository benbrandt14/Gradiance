use bevy::prelude::*;
use gradiance::input::editable_shape::{ShapeType, generate_shape_components};
...
fn test_box_invalid_zero() {
    ...
    assert!(
        result.is_none(),
        "Zero width box should not return components"
    );
    ...
    assert!(
        result.is_none(),
        "Zero height box should not return components"
    );
}
...
fn test_box_invalid_negative() {
    ...
    assert!(
        result.is_none(),
        "Negative width box should not return components"
    );
    ...
    assert!(
        result.is_none(),
        "Negative height box should not return components"
    );
}
...
fn test_circle_invalid_zero() {
    ...
    assert!(
        result.is_none(),
        "Zero radius circle should not return components"
    );
}
...
fn test_circle_invalid_negative() {
    ...
    assert!(
        result.is_none(),
        "Negative radius circle should not return components"
    );
}
...
fn test_polygon_valid() {
    ...
    assert!(
        result.is_some(),
        "Valid polygon (3 points) should return components"
    );
}
...
fn test_polygon_invalid_too_few_points() {
    ...
    assert!(
        result.is_none(),
        "Polygon with 2 points should not return components"
    );
}
