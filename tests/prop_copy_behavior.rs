...
fn test_copy_logic_extrusion_behavior() {
    ...
    let bad_entity = app.world_mut().spawn((
        EditableShape { shape: shape.clone() },
        // NO ExtrudableShape
    ...
    assert!(app.world().get::<Mesh3d>(bad_entity).is_none(), "Buggy duplicate should have no mesh");
    ...
    let good_entity = app.world_mut().spawn((
        EditableShape { shape: shape.clone() },
    ...
    assert!(app.world().get::<Mesh3d>(good_entity).is_some(), "Fixed duplicate SHOULD have mesh");
    let mesh_handle_1 = app.world().get::<Mesh3d>(good_entity).unwrap().0.clone();

    // 3. Test Property: Changing Layer updates Mesh
    app.world_mut().entity_mut(good_entity).insert(CollisionGroups::new(Group::GROUP_2, Group::GROUP_2));
    app.update();

    let mesh_handle_2 = app.world().get::<Mesh3d>(good_entity).unwrap().0.clone();
    assert_ne!(mesh_handle_1, mesh_handle_2, "Mesh handle should change when collision group changes");
}
