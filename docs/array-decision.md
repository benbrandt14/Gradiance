...
## What was carved back out

The tool briefly grew per-copy tweens: one lane per pattern axis, a per-axis
size ratio compounding down the array, spin and depth stepping, and a
closed-form tapered contact pitch so shrinking copies stayed flush (the
geometric-series result — copy `k` at `Q·(1 + u + … + u^{k-1})` — was correct
and exact for axis-aligned drags). A Lisp verb `(array-repeat …)` reached the
same intent from the script layer.

All of it is gone. Not because the maths was wrong, but because the feature
was never polished enough to trust, and carrying it made everything around it
harder:

- a `CopyPlacement` stopped being a translation and became a rotate-scale-
  translate with a frame and an origin, so the ghost needed a matching shear
  decomposition to avoid drifting from the committed result;
- the contact pitch stopped being one measured number and became a function
  of the copy index, which pulled the taper into the count computation, the
  spacing rules, and the grid's cross terms;
- `ArrayConfig` grew a nested two-lane struct that the panel had to render
  and `sanitized` had to clamp field by field.

The part that works — repeat a selection at a flush pitch — is worth building
on. The part that did not land is better re-approached from a clean base than
maintained in place. The reasoning above is kept so the next attempt starts
from it rather than rediscovering it; the code is in git.
