...
use crate::selection::{SelectTransition, SelectedJoint, Selection, expand_groups};
...
use gradiance_physics::grab::{Grab, MouseSpring, MouseTwist, Twist};
use gradiance_physics::hold::KinematicHold;
...
fn body_fully_inside(shape: &ShapeDef, transform: &Transform, min: Vec2, max: Vec2) -> bool {
    let (smin, smax) = gradiance_geometry::sdf::aabb(shape);
...
pub struct ManipInputs<'w> {
    ...
    cam_scale: Res<'w, crate::camera::CameraScale>,
}
