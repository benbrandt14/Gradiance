...
fn commit_tracers(
    selection: &Selection,
    props: &mut BodyProps,
    new: Option<gradiance_domain::tracer::Tracer>,
) {
    let changes: Vec<PropertyChange> = selection
        .iter()
        .filter_map(|e| {
...
fn commit_fields(
    selection: &Selection,
    props: &mut BodyProps,
    new: Option<gradiance_domain::field::FieldSource>,
) {
    let changes: Vec<PropertyChange> = selection
        .iter()
        .filter_map(|e| {
...
pub fn appearance_section(ui: &mut egui::Ui, selection: &Selection, props: &mut BodyProps) {
    ...
    let to_array = |c: gradiance_domain::appearance::Rgba| [c.r, c.g, c.b, c.a];
    let to_rgba = |c: [f32; 4]| gradiance_domain::appearance::Rgba {
        r: c[0],
        g: c[1],
        b: c[2],
        a: c[3],
