...
//! the read-total [`ToolWorld`] facade; the created joint leaves as a
//! [`ToolCommit::SpawnJoint`] through the shared commit seam.
...
impl ManipTool for ConnectorDraft {
    fn update(
        ...
        // Release: connect the topmost body/bodies at the anchor.
        if ctx.left == GesturePhase::Released
            && let Some(anchor) = self.0.take()
            && let Some(commit) = build_joint(ctx.tool, anchor, ctx.cursor.unwrap_or(anchor), world)
        {
